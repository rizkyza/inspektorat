...
class Lhp_temuan_rekomendasi extends CI_Controller
{
  ...
  {
    $this->data['module2'] = 'Data LHP Entry Rekomendasi Temuan';

    $row = $this->Temuan_model->get_by_id($id_temuan);
    $this->data['temuan'] = $this->Temuan_model->get_by_id($id_temuan);
    $nolhp = $this->data['temuan']->no_lhp;

    $rows = $this->Temuan_model->get_data_temuan_by_user($id_temuan);
    $id_lhp = $this->Lhp_model->get_id_by_nolhp($nolhp);

    if (!$rows)
    {
      $this->session->set_flashdata('message', 'Anda dilarang masuk untuk menambah atau mengubah rekomendasi.');
      redirect(site_url('admin/lhp_temuan/update/'.$id_lhp ));

    } else {

      if ($row)
      {
        $this->data['title']          = 'Entry Rekomendasi Temuan LHP';
        $this->data['action']         = site_url('admin/Lhp_temuan_rekomendasi/update_action');
        ...
        $this->data['get_combo_koder']=$this->Koderk_model->get_combo_koderk();
        $this->load->view('back/lhp/lhp_edit_temuan_rekomendasi', $this->data);
      }
      else
      {
  ...
  public function update_action()
    ...
    function fetch_user(){
         ...
         foreach($fetch_data as $row)
         {
              ...
              $sub_array[] = '<button type="button" name="update" id="'.$row->id_rekomendasi.'" class="btn btn-warning btn-xs update">Update</button>';
              $sub_array[] = '<button type="button" name="delete" id="'.$row->id_rekomendasi.'" class="btn btn-danger btn-xs delete">Delete</button>';
              $data[] = $sub_array;
         }
         $output = array(
    ...
    function fetch_single_user()
    {
         ...
         foreach($data as $row)
         {
              ...
              $output['id_temuanm'] = $row->id_temuan;
              $output['no_temuanm'] = $row->no_temuan;
              $output['judul_temuanm'] = $row->judul_temuan;
