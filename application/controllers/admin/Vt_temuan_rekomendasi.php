...
class Vt_temuan_rekomendasi extends CI_Controller
{
  function __construct()
  {
    ...
    $this->data['module'] = 'Data LHP Verifikasi Temuan';
  ...
  {
    $this->data['title'] = "Verifikasi Tindak Lanjut Rekomendasi Temuan LHP";

    $this->data['lhp_data'] = $this->Lhp_model->get_all_by_irban();
    $this->load->view('back/vt/vt_list_temuan', $this->data);
  }
  ...
  public function update($id_temuan)
  {
    ...
    if ($row)
    {
      ...
      $this->load->view('back/vt/vt_edit_temuan_rekomendasi', $this->data);
    }
      else
      {
  ...
  public function update_action()
    ...
    function fetch_user(){
         ...
         {
              $sub_array = array();
              $sub_array[] = $row->no_tindaklanjut;
              $sub_array[] = $row->kode_rekomendasi;
              $sub_array[] = $row->uraian_rekomendasi;
              $sub_array[] = $row->status;
              $sub_array[] = '<button type="button" name="update" id="'.$row->id_rekomendasi.'" class="btn btn-primary btn-sm update"><i class="glyphicon glyphicon glyphicon-share"> Verifikasi Tindak Lanjut</i></button>';
    ...
    function user_action(){
         ...
         if($_POST["action"] == "Verifikasi")
         {
              $updated_data = array(
                ...
                'time_verifikator'       => date('Y-m-d H:i:s')
              );
              $this->load->model('Rekomendasi_model');
