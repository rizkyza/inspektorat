...
class Subkoderk_model extends CI_Model
{
  public $table = 'sub_koderk';
...
function get_subkoderk($kode = null){
  if($kode != NULL){
    $this->db->where('kode', $kode);
  }
  $query = $this->db->get('sub_koderk');
  $data = array();
  if($query->result()){
    foreach ($query->result() as $row) {
      $data[0] = 'Pilih Sub Kode Rekomendasi';
  ...
  function get_combo_subkode_by_kode($kode)
  {
    ...
    if($query->num_rows() > 0){
      ...
      foreach ($query->result_array() as $row)
      {
        $data['subkode'] = 'Pilih Sub Kode Rekomendasi';
        $data[$row['subkode']] = $row['subkode'].' - '.$row['keterangan'];
      }
      return $data;
    }
